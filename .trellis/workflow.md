...
Then run `task.py start <task-dir>` to flip status to in_progress.
Research output **must** land in `{task_dir}/research/*.md`. In Codex inline mode, write research from the main session directly so the task binding stays available.
[/workflow-state:planning-inline]
